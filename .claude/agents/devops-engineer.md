...
- **Images:** `Dockerfile` only (distroless app; build context = repo root; `COPY . .` + root `.dockerignore`; build-args `VERSION/COMMIT/BUILD_DATE` → ldflags; migrations are embedded at compile time via `//go:embed` in `internal/platform/database/migrations/runner.go`).
...
- **Migrations run automatically** — embedded in the app binary (`internal/platform/database/migrations/runner.go`), applied at startup before the HTTP server starts. No separate migrate service or image.
- **Image build context is the repo root** — keep the root `.dockerignore` excluding the JS workspace; keep `internal/platform/observability/buildinfo` ldflags wired.
