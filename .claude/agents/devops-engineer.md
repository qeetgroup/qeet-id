...
- **Images:** `deploy/base/docker/Dockerfile` only (distroless app; build context = repo root; `COPY . .` + root `.dockerignore`; build-args `VERSION/COMMIT/BUILD_DATE` → ldflags; migrations are embedded at compile time via `//go:embed` in `platform/database/migrations/runner.go`).
- **Prod Compose:** `deploy/environments/prod/docker-compose.yml` (app + redis + caddy; no local Postgres — uses AWS RDS via `DB_URL`), `Caddyfile`, `.env.example`, `setup.sh`.
...
docker build -f deploy/base/docker/Dockerfile .
```
