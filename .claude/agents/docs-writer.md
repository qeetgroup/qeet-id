...
2. **API docs** — descriptions/examples in `api/openapi/` for new/changed endpoints (the schema itself is owned by `backend-engineer`; you refine prose/examples/descriptions). Keep every mounted route documented there — a manual expectation, not an enforced test.
