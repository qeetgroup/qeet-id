...
- **Dedup / current state (READ FIRST):** `/Users/a3097640/Desktop/QG/qeet-servers/qeet-id-server/ROADMAP.md` — the golden inventory of what Qeet ID already has + an existing competitor matrix. Also `/Users/a3097640/Desktop/QG/qeet-files/qeet-id/Product_Requirement_Document.md`.
...
- **Source code (REQUIRED dedup cross-check):** `/Users/a3097640/Desktop/QG/qeet-servers/qeet-id-server/` — Go monolith under `internal/` (the bounded contexts) + `internal/platform/`. Check `internal/platform/database/migrations/` (latest number = what really shipped) and `find internal -type d` (a package = a built capability). The status doc lags; the code is ground truth.
...
1. **Orient & dedupe — analyze the project FIRST, before touching the web.** Run `date`. Read `ROADMAP.md` (the stated inventory), the current `FEATURE-CATALOG.md`, the top ~2 entries of `COMPETITIVE-INTEL.md`, and `FEATURE-PROPOSALS.md`. **Then verify against the actual source — don't trust the status doc alone** (it lags reality). Cross-check with the code in `/Users/a3097640/Desktop/QG/qeet-servers/qeet-id-server/`:
   - `ls internal/platform/database/migrations/` — the highest migration number tells you what schema/features actually landed (each `NNNN_<name>` is a real feature).
   - `find internal -type d` — a package's existence (e.g. `internal/access/authorization/rebac`, `internal/developer/{agents,auth-hooks,credentials/vc}`, `internal/operations/siem`) proves the capability is built even if the status doc says ⏳.
