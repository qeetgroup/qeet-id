...
- **Crypto & secrets:** no secrets in code/logs/responses; correct use of `internal/platform/crypto/tokens` / the password-hashing package `internal/platform/crypto/encryption` (`package password`) / hashing; constant-time compares for tokens; no `*.pem`/keys committed.
