...
Qeet ID issues **ES256 JWTs** (ECDSA P-256, asymmetric). All tokens are signed with the private key held in `platform/tokens`; the corresponding public key is published at `/jwks.json` for external verification (e.g., OIDC relying parties).
...
2. Add it to the active key set in `platform/tokens`.
