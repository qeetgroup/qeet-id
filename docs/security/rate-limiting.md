...
Rate limiting is implemented in `platform/ratelimit` using a **token-bucket algorithm**. Limits are applied per key type with different parameters for different principal types and endpoints.
