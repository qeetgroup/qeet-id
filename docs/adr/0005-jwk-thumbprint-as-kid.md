...
Implementation: `platform/tokens/jwt.go` — the `kid` is computed once at key load time and embedded in the signing header.
...
2. Add new key to active set in `platform/tokens`
