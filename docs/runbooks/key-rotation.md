...
In `platform/tokens`, the key rotation design allows multiple active keys. Add the new key to the configuration:
...
To minimize user disruption: deploy during low-traffic hours, or implement a rolling key (primary + previous key both accepted) — this requires a minor code change to `platform/httpx/csrf.go`.
