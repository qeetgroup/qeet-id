...
The tenant ID is extracted from the authenticated JWT claim (`tid`) in `platform/httpx/auth.go` and stored in the request context as part of the `httpx.Principal`:
