...
- **Docker build context is the repo root** (single Go module), though the Dockerfile lives under [deploy/base/docker/](deploy/base/docker/) — build with `-f deploy/base/docker/Dockerfile .`. The root [.dockerignore](.dockerignore) excludes the JS workspace + `deploy/`; migration SQL files are embedded in the binary via `//go:embed` in `platform/database/migrations/runner.go` and run automatically on startup.
